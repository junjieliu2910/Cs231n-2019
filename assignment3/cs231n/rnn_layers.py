...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    next_h = h0
    for i in range(T):
      next_h, temp = rnn_step_forward(x[:, i, :], next_h, Wx, Wh, b)
      cache.append(temp)
      h.append(next_h)
    h = np.stack(h, axis=1)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
      dx[:, i, :], dprev_hc, dWxc, dWhc, dbc = rnn_step_backward(dh[:, i, :]+dh0, cache[i])
      dh0 = dprev_hc
      dWx += dWxc 
      dWh += dWhc 
      db += dbc 
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    next_h = h0
    for i in range(T):
      next_h, cell_state, temp = lstm_step_forward(x[:, i, :], next_h, cell_state, Wx, Wh, b)
      h[:, i, :] = next_h
      cache.append(temp)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
...
def lstm_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
      dx[:, i, :], dnext_h, dnext_c, dWxc, dWhc, dbc = lstm_step_backward(dh[:, i, :]+dnext_h, dnext_c, cache[i])
      dh0 = dnext_h
      dWx += dWxc 
      dWh += dWhc 
      db += dbc  
